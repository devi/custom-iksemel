...
typedef void (iksTClose)(void *user_data);
typedef void (iksTConnect)(void *user_data, const char *server, int port);
typedef void (iksTSend)(void *user_data, const char *data, size_t len);
typedef void (iksTlsHandshake)(void *user_data);
...
typedef int (iksXmppPacketHook)(void *user_data, ikspak *pak);
typedef void (iksLogHook)(void *user_data, const char *buf, size_t size, int type);

iksparser *xmpp_init_parser (void *user_data, iksXmppPacketHook *xmppHook, iksLogHook *logHook);
iksid *xmpp_set_jid (iksparser *prs, char *jid, char *passwd);
int xmpp_connect (iksparser *prs, const char *server, int port, ikstransport *trans);
void xmpp_disconnect (iksparser *prs);

int xmpp_send_xml (iksparser *prs, const char *xmlstr);
int xmpp_send (iksparser *prs, iks *x);
int xmpp_send_header (iksparser *prs, const char *to);

int xmpp_proses_data (iksparser *prs, const char *buf, size_t len);

int xmpp_is_secure (iksparser *prs);
//int xmpp_start_tls (iksparser *prs);
int xmpp_tls_done (iksparser *prs);

//int xmpp_start_sasl (iksparser *prs, enum ikssasltype type);

void xmpp_set_basic_hook (iksparser *prs, iksFilterHook *iqHook, iksFilterHook *presHook, iksFilterHook *msgHook, iksFilterHook *s10nHook);

// from climm.org
iks *iks_find_with_ns_attrib (iks *x, const char *childname, const char *childnamespace);


typedef struct xmpp_callbacks_struct {
	
} xmmpp_callbacks;

//void xmpp_set_callbacks
